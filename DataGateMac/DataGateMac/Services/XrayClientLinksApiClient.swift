//
//  XrayClientLinksApiClient.swift
//  DataGateMac
//
//  Same shapes as OpenVPN files: download-file-by-cn / add-with-token on /api/xray-client-links.
//

import Foundation
import os

private let log = Logger(subsystem: "imkolganov.DataGateMac", category: "XrayClientLinksApi")

final class XrayClientLinksApiClient {
    private init() {}
    static let shared = XrayClientLinksApiClient()
    ...
    func ensureAndDownloadDeviceFile(
        ...
        guard let second = try await tryDownload(vpnServerId: vpnServerId, commonName: commonName, token: token) else {
            throw ApiClientError.custom("Xray client link not found after create")
        }
        return second
    }
    ...
    private func tryDownload(vpnServerId: Int, commonName: String, token: String) async throws -> DownloadFileResponse? {
        ...
        let url = URL(string: "\(baseUrl)/api/xray-client-links/download-file-by-cn")!
    ...
    private func addWithToken(
        ...
        let config = try AppConfig.load()
        let baseUrl = config.apiBaseUrl.trimmingCharacters(in: CharacterSet(charactersIn: "/"))
        let url = URL(string: "\(baseUrl)/api/xray-client-links/add-with-token")!
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
